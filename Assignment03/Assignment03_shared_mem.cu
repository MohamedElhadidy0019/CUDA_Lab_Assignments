...
int main() {
    ...
    for (auto num_of_points : num_of_points_list) {
        ...
        // Measure runtime for Cuda shared memory method
        start = std::chrono::high_resolution_clock::now();
        float result_cuda_shared = chamfer_distance_cuda_shared(cloud1, cloud2);
        end = std::chrono::high_resolution_clock::now();
        std::cout << "Chamfer Distance (Cuda Shared Memory): " << result_cuda_shared
            << ", Time: " << std::chrono::duration<double>(end - start).count() << " seconds\n";
        ...
        float rel_err_cuda = std::fabs(result_cuda - ref) / (std::fabs(ref) + 1e-8f);
        float rel_err_shared = std::fabs(result_cuda_shared - ref) / (std::fabs(ref) + 1e-8f);
        std::cout << "Consistency Check (CUDA vs " << ref_name << "): "
            << (rel_err_cuda < 1e-3f ? "PASSED" : "FAILED")
            << " (relative error = " << rel_err_cuda << ")\n";
        std::cout << "Consistency Check (CUDA Shared vs " << ref_name << "): "
            << (rel_err_shared < 1e-3f ? "PASSED" : "FAILED")
            << " (relative error = " << rel_err_shared << ")\n";
        std::cout << "------------------------------------------------------------\n";
    }
